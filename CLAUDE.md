...
**coresapian.com** is a multi-surface interactive experience: a static web app deployed on Netlify, a Godot 4.6 web build at `/game/`, and a Godot 4.6 + SwiftUI native iOS app. There is no backend server; everything runs client-side.
...
# Regenerate the iOS Xcode project if project.yml changes
...
```
public/           # Web deploy root — Netlify serves this directly
  rune_puzzle/    # Puzzle (Three.js, GSAP, ES modules)
  core_truths_book/ # Book carousel + oracle terminal (ONNX worker)
  game/           # Godot Web export + custom shell at /game/
godot/            # Godot 4.6 project (Mobile renderer, landscape 1280x720)
  scenes/         # .tscn + .gd files (main, rune_puzzle, core_truths)
  shaders/        # matrix_rain, godrays, glow (.gdshader)
ios/              # SwiftUI + SwiftGodotKit native iOS wrapper
  CoreSapian/     # App entry, ContentView, bridge, asset catalog
  project.yml     # XcodeGen source of truth
  CoreSapian.xcodeproj/ # Generated and checked in for direct Xcode use
```
...
- **`project.yml`** + **`CoreSapian.xcodeproj`** — Native project definition with automatic package resolution and a pre-build `main.pck` export step.
- iOS runtime target is **iOS 17+ on physical devices** due current `SwiftGodotKit` constraints.
