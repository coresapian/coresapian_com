...
function animate() {
    ...
    if (isPreloaderActive) {
        ...
        // if (anomalyMaterial) anomalyMaterial.uniforms.u_time.value = elapsedTime; // Commented out u_time update
...
function initMainScene() {
    ...
    scene.fog = new THREE.FogExp2(0x12100f, 0.2);
    ...
    if (cachedSingularityModel) {
        ...
        anomalyObject.traverse((child) => {
            if (child.isMesh) {
                // child.material = anomalyMaterial; // Display model with original materials
                if (child.material) {
                    child.material.depthWrite = true; // Ensure depth writing is enabled for original materials
                    child.material.visible = true;    // Ensure it's visible
...
function updateAnomalyShaderAudio() {
    ...
    // anomalyMaterial.uniforms.u_bass.value = (bassSum / bassCutoff / 255); // Commented out u_bass update

    let midSum = 0;
    for (let i = bassCutoff; i < midCutoff; i++) midSum += frequencyData[i];
    // anomalyMaterial.uniforms.u_mid.value = (midSum / (midCutoff - bassCutoff) / 255); // Commented out u_mid update

    let trebleSum = 0;
    for (let i = midCutoff; i < frequencyData.length; i++) trebleSum += frequencyData[i];
    // anomalyMaterial.uniforms.u_treble.value = (trebleSum / (frequencyData.length - midCutoff) / 255); // Commented out u_treble update
}
